...
function renderCategoryLabel(box: LayoutBox, item: InfographicItem, color: string, muted: string, align: "start" | "middle" | "end" = "middle") {
  const x = align === "start" ? box.x + 4 : align === "end" ? box.x + box.width - 4 : box.x + box.width / 2;
  const lines = wrapVisualText(item.title, charsForWidth(Math.max(60, box.width - 8), 10.5), 2).lines;
  const y = box.y + 18 - Math.max(0, lines.length - 1) * 6;
  return (
    <text x={x} y={y} textAnchor={align} fill={muted} fontSize="10.5" fontWeight="700">
  ...
  return (
    <VisualFrame plan={plan} data={data} style={style} label="Barres">
      ...
      {Array.from({ length: 5 }, (_, index) => {
        ...
        return (
          <g key={`grid-${index}`}>
            <line x1={x} x2={x} y1={plot.y} y2={plot.y + plot.height} stroke={theme.text} strokeOpacity="0.08" />
      ...
      {labels.map((label, index) => {
        ...
        const barHeight = Math.min(32, Math.max(16, label.height * 0.46));
        const y = label.y + (label.height - barHeight) / 2;
        const color = itemColor(index, theme.palette, theme.accent);
        return (
          <g key={`bar-${index}`} data-box-id={label.id}>
            {renderCategoryLabel(label, item, color, theme.muted, "end")}
            <rect x={x} y={y} width={width} height={barHeight} rx={barHeight / 2} fill={color} opacity="0.84" />
            <text
              x={value >= 0 ? xValue + 10 : xValue - 10}
              y={y + barHeight / 2 + 4}
...
function cartesianScale(items: InfographicItem[], plot: LayoutBox) {
  ...
  return { values, min, max, range, yFor, zeroY: yFor(0) };
}
  ...
  return (
    <>
      {Array.from({ length: 5 }, (_, index) => {
        ...
        return (
          <g key={`grid-${index}`}>
            <line x1={plot.x} x2={plot.x + plot.width} y1={y} y2={y} stroke={theme.text} strokeOpacity="0.08" />
...
export function ColumnVisual({ data, style }: Props) {
  ...
  const { values, min, range, yFor, zeroY } = cartesianScale(chartData.items, plot);
  ...
  return (
    <VisualFrame plan={plan} data={data} style={style} label="Colonnes">
      ...
      <line x1={plot.x} x2={plot.x + plot.width} y1={zeroY} y2={zeroY} stroke={theme.text} strokeOpacity="0.32" strokeWidth="1.4" />
      {chartData.items.map((item, index) => {
        ...
        return (
          <g key={`column-${index}`}>
            <rect x={x} y={y} width={width} height={height} rx="8" fill={color} opacity="0.84" />
            <text x={x + width / 2} y={value >= 0 ? y - 9 : y + height + 16} textAnchor="middle" fill={theme.text} fontSize="10.5" fontWeight="850">{formatNumericValue(item)}</text>
            {label && renderCategoryLabel(label, item, color, theme.muted)}
          </g>
        );
      })}
    </VisualFrame>
...
export function LineVisual({ data, style }: Props) {
  ...
  return (
    <VisualFrame plan={plan} data={data} style={style} label="Courbe">
      <Grid plot={plot} min={min} range={range} yFor={yFor} theme={theme} />
      <line x1={plot.x} x2={plot.x + plot.width} y1={zeroY} y2={zeroY} stroke={theme.text} strokeOpacity="0.22" />
      ...
      {points.map((point, index) => {
        ...
        return (
          <g key={`line-${index}`}>
            ...
            <text x={point.x} y={point.y - 15} textAnchor="middle" fill={theme.text} fontSize="10.5" fontWeight="850">{formatNumericValue(item)}</text>
            {label && renderCategoryLabel(label, item, color, theme.muted)}
          </g>
        );
      })}
      ...
      {legend.map((box, index) => {
        ...
        return (
          <g key={`donut-legend-${index}`} data-box-id={box.id}>
            <rect x={box.x} y={box.y} width={box.width} height={box.height} rx="14" fill={color} fillOpacity={theme.dark ? 0.1 : 0.055} />
            <circle cx={box.x + 18} cy={box.y + box.height / 2} r="6" fill={color} />
            <text x={box.x + 34} y={box.y + box.height / 2 - Math.max(0, labelLines.length - 1) * 7 + 4} fill={theme.text} fontSize="11.5" fontWeight="750">
              {labelLines.map((line, lineIndex) => <tspan key={`${line}-${lineIndex}`} x={box.x + 34} dy={lineIndex === 0 ? 0 : 14}>{line}</tspan>)}
            </text>
            <text x={box.x + box.width - 14} y={box.y + box.height / 2 - 4} textAnchor="end" fill={color} fontSize="11" fontWeight="850">{formatNumericValue(item)}</text>
  ...
  return (
    <VisualFrame plan={plan} data={data} style={style} label="Waterfall chiffré">
      ...
      {chartData.items.map((item, index) => {
        ...
        const color = delta >= 0 ? theme.palette[0] ?? theme.accent : theme.palette[Math.min(2, theme.palette.length - 1)] ?? theme.accent;
        const label = labels[index];
        return (
          <g key={`waterfall-${index}`}>
            ...
            <rect x={x} y={y} width={width} height={height} rx="7" fill={color} opacity="0.86" />
            <text x={x + width / 2} y={delta >= 0 ? y - 10 : y + height + 16} textAnchor="middle" fill={theme.text} fontSize="10.5" fontWeight="850">{formatNumericValue(item)}</text>
            {label && renderCategoryLabel(label, item, color, theme.muted)}
          </g>
        );
      })}
